...
app.get('/api/v1/fruits/', async(req, res, next) => {
  try {
    const createdFruit = await Fruit.find(req);
    res.send(createdFruit); 
  } catch(error) {
    next(error);
  }
...
app.put('/api/v1/fruits/:id', async(req, res, next) => {
  try {
    const createdFruit = await Fruit.update(req.params.id, req.body);
    res.send(createdFruit); 
  } catch(error) {
    next(error);
  }
